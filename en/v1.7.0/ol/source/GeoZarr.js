 ...
 */
import { FetchStore, NotFoundError, get, open, slice, withRangeCoalescing, } from 'zarrita';
...
export default class GeoZarr extends DataTileSource {
    ...
    constructor(options) {
        ...
        for (const name in this.dimensions_) {
            ...
            if (!this.variable_) {
                throw new Error(`GeoZarr: dimension "${name}" selects several bands, which requires ` +
                    'the `variable` option.');
            }
            if (multiName) {
                throw new Error(`GeoZarr: dimensions "${multiName}" and "${name}" both select ` +
        ...
        this.configure_()
            ...
            this.error_ =
                err instanceof NotFoundError
                    ? new Error(`GeoZarr: could not read ${this.url_}; it is missing, or ` +
                        `access to it is denied.`, { cause: err })
                    : err;
            this.setState('error');
        });
    }
    async configure_() {
        ...
        const store = /** @type {FetchStore} */ (withRangeCoalescing(new FetchStore(this.url_, { overrides, fetch: storeFetch })));
        ...
        if (this.groupPaths_) {
            ...
            for (const groupPath of this.groupPaths_) {
                groupPromises.push(this.openFn_(rootGroup.resolve(groupPath), { kind: 'group' }));
            }
        }
        else {
            // Single group mode
        ...
        const hasMultiscaleLevels = Array.isArray(multiscales) ||
            Array.isArray(/** @type {Multiscales} */ (_a = (multiscales)) === null || _a === void 0 ? void 0 : _a.layout);
        ...
        if (!this.variable_) {
            for (const [name, value] of Object.entries(this.dimensions_)) {
                if (typeof value === 'string') {
                    this.dimensions_[name] = await this.resolveCoordinateLabel_(name, value, (_c = (_b = this.coordinateArray_(name)) === null || _b === void 0 ? void 0 : _b.path) !== null && _c !== void 0 ? _c : name);
                }
            }
        }
    ...
    resolveExtraSelection_(arrayMeta, dimensions) {
        ...
        for (const dim of dims) {
            ...
            if (typeof index === 'string') {
                ...
                throw new Error(`GeoZarr: coordinate labels cannot be passed to updateDimensions; ` +
    ...
    async configureLevels_(attributes, store) {
        ...
        for (const raw of rawLevels) {
            const bands = consolidatedMetadata
                ? arrayNames.filter((name) => consolidatedMetadata[arrayPathOf(raw.path, name)])
                : arrayNames.slice();
            if (bands.length === 0) {
                warn(`No ${what} found at level "${raw.path || '.'}"`);
                continue;
            }
            levels.push(Object.assign(Object.assign({}, raw), { bands, meta: consolidatedMetadata
        ...
        if (!meta && (variable || !levels.every((l) => Array.isArray(l.shape)))) {
            meta = await getArrayMeta(store, arrayPathOf(levels[0].path, levels[0].bands[0]), this.openFn_ === open.v2);
            levels[0].meta = meta;
        }
        ...
        /** @type {Array<{path: string, bands: Array<string>, meta: any, shape: Array<number>|undefined, resolution: number, rowResolution: number, origin: import("ol/coordinate.js").Coordinate}>} */
        const configured = [];
        for (const level of levels) {
            let resolution;
            // Pixels are not necessarily square (e.g. a square array covering the
            ...
            for (let axis = 0; axis < ndim; ++axis) {
                ...
                for (const v of values) {
                    if (typeof v === 'number') {
                        ...
                        indices.push(await this.resolveCoordinateLabel_(dimName, v, arrayPathOf(finestPath, dimName)));
                    }
                }
