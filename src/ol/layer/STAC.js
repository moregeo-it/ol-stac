...
class STACLayer extends LayerGroup {
      ...
      case 'tilejson': {
        ...
        if (isObject(tileJSON) && Array.isArray(tileJSON.tiles)) {
          ...
          tileJSON.tiles = tileJSON.tiles.map((template) =>
            this.getRequestUrlFor_(template, link, true),
          );
        }
  ...
  checkDisplayLimit_(source, asset, autoDisplay) {
    ...
    if (!autoDisplay) {
      ...
      const error = new Error(
        `Asset ${asset.getKey()} is too large to display safely` +
  ...
  async addGeoZarr_(asset, autoDisplay = false) {
    ...
    const declaresRendering = () =>
      asset.isType(wozMediaTypes) ||
    ...
    try {
      if (!declaresRendering()) {
        throw new Error(
          `Asset ${asset.getKey()} declares neither bands nor a datacube variable to render`,
        );
      }
