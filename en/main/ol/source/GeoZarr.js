...
export default class GeoZarr extends DataTileSource {
    ...
    constructor(options) {
        super({
            state: 'loading',
            tileGrid: /** @type {?} */ (null),
            projection: /** @type {import("ol/proj.js").ProjectionLike} */ (options.projection || null),
        ...
        if (this.variable_) {
            ...
            for (const key in this.selector_) {
                ...
                if (Array.isArray(value)) {
                    if (bandCount > 1) {
                        throw new Error('Only one selector dimension may select multiple values');
                    }
    ...
    async configure_() {
        ...
        const store = /** @type {FetchStore} */ (withRangeCoalescing(new FetchStore(this.url_, { overrides })));
        ...
        const cachedStore = v2Metadata
            ...
            : this.consolidatedMetadata_
                ? createCachedStore(store, 
                /** @type {Uint8Array} */ (groupBytes), this.consolidatedMetadata_)
        ...
        const attributes = 
        /** @type {LegacyDatasetAttributes | DatasetAttributes} */ (this.groups_[0].attrs);
        // The spatial: convention names the two spatial axes (`[y, x]`), so they can
        // be located by name in each array's `dimension_names`.
        const spatialDimensions = 'spatial:dimensions' in attributes
        ...
        const consolidatedMetadata = this.groupPaths_ && this.consolidatedMetadata_
        ...
        if (!this.tileGrid &&
            ...
            REQUIRED_ZARR_CONVENTIONS.every((uuid) => attributes.zarr_conventions.find((c) => c.uuid === uuid)) &&
            'layout' in attributes.multiscales) {
            const { tileGrid, projection, bandsByLevel, fillValue, tileSizes } = getTileGridInfoFromAttributes(
            /** @type {DatasetAttributes} */ (attributes), consolidatedMetadata, this.bands_);
        ...
        if (!this.tileGrid && 'spatial:bbox' in attributes) {
            ...
            let xSize = (_a = attributes['spatial:shape']) === null || _a === void 0 ? void 0 : _a[1];
            if (xSize === undefined && consolidatedMetadata) {
                for (const band of this.bands_) {
                    const bandMeta = consolidatedMetadata[band];
                    if (bandMeta === null || bandMeta === void 0 ? void 0 : bandMeta.shape) {
                    ...
                    for (const band of this.bands_) {
                        if (consolidatedMetadata[band]) {
                            ...
                            if (this.fillValue_ === undefined) {
                                this.fillValue_ = Number(consolidatedMetadata[band]['fill_value']);
                            }
                        }
                    }
                }
        ...
        const extent = /** @type {import("ol/extent.js").Extent} */ (this.tileGrid.getExtent());
        const projection = /** @type {import("ol/proj/Projection.js").default} */ (this.projection);
        setTimeout(() => {
            var _a;
            (_a = this.viewResolver) === null || _a === void 0 ? void 0 : _a.call(this, {
                showFullExtent: true,
    ...
    async loadTile_(z, x, y, options) {
        ...
        for (let i = 0, ii = this.bands_.length; i < ii; ++i) {
            ...
            if (!this.bandsByLevel_) {
                ...
                for (let candidateZ = 0; candidateZ < resolutions.length; candidateZ += 1) {
            ...
            const effectiveResolution = /** @type {number} */ (bandResolution !== null && bandResolution !== void 0 ? bandResolution : resolvedBandResolution);
            // Pixels are not necessarily square (e.g. a square array covering the
            // full EPSG:4326 world), and rows may be stored south-up.
            const rowInfo = (_a = this.levelRowInfo_) === null || _a === void 0 ? void 0 : _a[bandMatrixId];
            const rowResolution = !isSingleScale && rowInfo ? rowInfo.rowResolution : effectiveResolution;
            const flip = !!(rowInfo === null || rowInfo === void 0 ? void 0 : rowInfo.flip) && rowInfo.shapeY !== undefined;
    ...
    resolveBandOwnership_() {
        var _a;
        const bandsByLevel = /** @type {Object<string, Array<string>>} */ (this.bandsByLevel_);
        const groupPaths = /** @type {Array<string>} */ (this.groupPaths_);
        const subMetadatas = groupPaths.map((gp) => getSubMetadata(
        /** @type {Object<string, *>} */ (this.consolidatedMetadata_), gp));
        for (let i = 0, ii = this.bands_.length; i < ii; ++i) {
            const band = this.bands_[i];
            ...
            for (const matrixId of Object.keys(bandsByLevel)) {
                const bandMeta = /** @type {Object<string, *>|undefined} */ (subMetadatas[g][`${matrixId}/${band}`]);
                if (bandMeta) {
            ...
            if (!foundAtAnyLevel) {
                ...
                const bandMeta = /** @type {Object<string, *>|undefined} */ (subMetadatas[g][band]);
                if (bandMeta) {
                    for (const matrixId of Object.keys(bandsByLevel)) {
                        if (!bandsByLevel[matrixId].includes(band)) {
                            bandsByLevel[matrixId].push(band);
                    ...
                    const shape = /** @type {Array<number>|undefined} */ (bandMeta['shape']);
                    const xSize = shape && shape[this.axesOf_(bandMeta).col];
                    if (xSize !== undefined && xSize > 0) {
                        const tileExtent = /** @type {import("ol/extent.js").Extent} */ (this.tileGrid.getExtent());
            ...
            if (!foundAtAnyLevel) {
                warn(`Band "${band}" from group "${(_a = this.groupPaths_) === null || _a === void 0 ? void 0 : _a[g]}" is not available at any ` +
    ...
    openArray_(groupIndex, path) {
        ...
        if (!array) {
            array =
                /** @type {Promise<import('zarrita').Array<import('zarrita').DataType, any>>} */ (this.openFn_(this.groups_[groupIndex].resolve(path), {
                    kind: 'array',
    ...
    groupMetadata_(groupIndex) {
        return this.groupPaths_
            ? getSubMetadata(
            /** @type {Object<string, *>} */ (this.consolidatedMetadata_), this.groupPaths_[groupIndex])
    ...
    coordinateArray_(name) {
        ...
        for (const path of Object.keys(meta)) {
            if (path === name || path.endsWith(suffix)) {
                ...
                if (Array.isArray(arrayMeta === null || arrayMeta === void 0 ? void 0 : arrayMeta.shape) && arrayMeta.shape.length === 1) {
    ...
    async getDimensions() {
        ...
        for (const dimension of this.extraDimensions_) {
            ...
            dimensions[dimension.name] = {
                size: dimension.size,
                attributes: coord ? ((_a = coord.meta.attributes) !== null && _a !== void 0 ? _a : null) : null,
            };
        }
        return dimensions;
    }
    ...
    async getValue(name, index) {
        ...
        if (index < 0 || index >= size) {
            throw new Error(`GeoZarr: index ${index} out of range for dimension "${name}" (size ${size}).`);
        }
    ...
    updateDimensions(dimensions) {
        const merged = Object.assign(Object.assign({}, this.dimensions_), dimensions);
        ...
        const selection = this.bands_.map((band, i) => {
            ...
            for (const [name, index] of Object.entries(dimensions)) {
                ...
                if (!dim || !updated) {
                    throw new Error(`GeoZarr: unknown dimension "${name}"; ` +
                        `available: [${dims.map((d) => d.name).join(', ')}].`);
                }
                if (dim.axis === this.multiAxis_) {
                    throw new Error(`GeoZarr: dimension "${name}" selects the rendered bands; ` +
                ...
                if (typeof index !== 'number' ||
                    ...
                    index < 0 ||
                    index >= dim.size) {
                    throw new Error(`GeoZarr: invalid index ${index} for dimension "${name}" ` +
                        `(size ${dim.size}).`);
                }
    ...
    extraDimsOf_(arrayMeta) {
        ...
        for (const axis of this.axesOf_(arrayMeta).extra) {
            ...
            const name = raw === null || raw === undefined ? String(axis) : String(raw);
    ...
    resolveExtraSelection_(arrayMeta, dimensions) {
        ...
        for (const key of dimKeys) {
            if (!names.includes(key) && !singleUnnamed) {
                throw new Error(`GeoZarr: unknown dimension "${key}" in the \`dimensions\` option; ` +
        ...
        for (const dim of dims) {
            ...
            if (typeof index === 'string') {
                ...
                throw new Error(`GeoZarr: datetime-label selection for dimension "${name}" is not yet ` +
    ...
    async configureDatacube_(attributes, store) {
        ...
        for (const raw of rawLevels) {
            ...
            levels.push(Object.assign(Object.assign({}, raw), { arrayPath, meta }));
        }
        if (levels.length === 0) {
            throw new Error(`Variable "${variable}" not found in any level`);
        }
        // The dimension layout is the same for all levels
        let meta = (_a = levels.find((level) => level.meta)) === null || _a === void 0 ? void 0 : _a.meta;
        if (!meta) {
        ...
        if (!dimensionNames) {
            throw new Error(`Cannot determine dimension names for variable "${variable}"`);
        }
        ...
        if (extent &&
            ...
            try {
                ...
                if (!extent && !this.fallbackExtent_) {
                    throw new Error(`Could not determine the extent of variable "${variable}" ` +
        ...
        for (const level of levels) {
            ...
            let rowResolution;
            let origin;
            if (Array.isArray(level.transform) &&
        ...
        for (let axis = 0; axis < ndim; ++axis) {
            ...
            let value = dimName in this.dimensions_
                ? this.dimensions_[dimName]
                : this.selector_[dimName];
            if (value === undefined) {
                warn(`No selector value given for dimension "${dimName}", using index 0.`);
                value = 0;
            }
            ...
            for (const v of values) {
                if (typeof v === 'number') {
                    ...
                    indices.push(await this.resolveCoordinateLabel_(dimName, v, finestPath));
                }
            }
            for (const index of indices) {
                if (!Number.isInteger(index) ||
                    ...
                    throw new Error(`GeoZarr: invalid index ${index} for dimension "${dimName}" ` +
        ...
        const tileSizes = configured.map((level) => {
            ...
            if (shardInfo) {
                return /** @type {import("ol/size.js").Size} */ ([
                    getTileSizeForShard(shardInfo.shardShape[1], shardInfo.innerChunkShape[1]),
                    getTileSizeForShard(shardInfo.shardShape[0], shardInfo.innerChunkShape[0]),
                ]);
            }
            const chunkShape = (_b = (_a = level.meta['chunk_grid']) === null || _a === void 0 ? void 0 : _a.configuration) === null || _b === void 0 ? void 0 : _b.chunk_shape;
        ...
        for (let i = 0; i < configured.length; ++i) {
            ...
            this.levelRowInfo_[matrixIds[i]] = {
                ...
                shapeY: level.meta && Array.isArray(level.meta.shape)
                    ? level.meta.shape[row]
                    : undefined,
                flip: flipY,
            };
        }
        this.tileGrid = new WMTSTileGrid(Object.assign({ extent: extent, origins: configured.map((level) => level.origin), resolutions: configured.map((level) => level.resolution), matrixIds: matrixIds }, (hasTileSizes
    ...
    inferProjection_(attributes, crsHint, extent) {
        ...
        if (typeof definition === 'string') {
            try {
                ...
                warn(`Could not register the proj4 definition: ${ /** @type {Error} */(err).message}`);
            }
        }
        if (extent &&
            extent[0] >= -360 &&
            ...
            return /** @type {import('ol/proj/Projection.js').default} */ (getProjection('EPSG:4326'));
        }
    ...
    async resolveCoordinateLabel_(dimName, label, levelPath = '') {
        try {
            ...
            if (index < 0) {
                throw new Error(`Label not found. Available: ${values.slice(0, 10).join(', ')}`);
            }
            return index;
        }
        catch (err) {
            throw new Error(`Could not resolve label "${label}" for dimension "${dimName}": ` +
...
function createCachedStore(store, groupBytes, consolidatedMetadata) {
    ...
    for (const [key, value] of Object.entries(consolidatedMetadata)) {
        cache.set(
        /** @type {`/${string}`} */ (`/${key}/zarr.json`), encoder.encode(JSON.stringify(value)));
    }
    return withChunkCache({
        ...
        getRange: (_a = store.getRange) === null || _a === void 0 ? void 0 : _a.bind(store),
    });
}
/***
 * @typedef {{
...
function getSpatialAxes(spatialDimensionNames, arrayMeta) {
    ...
    if (Array.isArray(spatialDimensionNames) &&
...
function getTileGridInfoFromAttributes(attributes, consolidatedMetadata, wantedBands) {
    ...
    for (const groupMetadata of multiscales.layout) {
        ...
        const spatialShape = /** @type {Array<number>} */ (groupMetadata['spatial:shape']);
        ...
        if (consolidatedMetadata && bandsByLevel) {
            ...
            for (const band of wantedBands) {
                const bandArray = /** @type {Object<string, *>|undefined} */ (consolidatedMetadata[`${matrixId}/${band}`]);
                if (bandArray) {
    ...
    const hasTileSizes = tileSizes.some((s) => s !== undefined);
    const tileGrid = new WMTSTileGrid(Object.assign({ extent: extent, origins: groupInfo.map((g) => g.origin), resolutions: groupInfo.map((g) => g.resolution), matrixIds: groupInfo.map((g) => g.matrixId) }, (hasTileSizes
        ? {
            tileSizes: /** @type {Array<import("ol/size.js").Size>} */ (tileSizes.map((s) => s || [256, 256])),
        }
        : {})));
    return {
        tileGrid,
        ...
        tileSizes: hasTileSizes
            ? /** @type {Array<import("ol/size.js").Size>} */ (tileSizes.map((s) => s || [256, 256]))
            : undefined,
    };
}
/**
...
function getTileGridInfoFromLegacyAttributes(attributes) {
    ...
    const info = parseTileMatrixSet({}, tileMatrixSet, undefined, tileMatrixLimits);
    let tileGrid = info.grid;
    // Tile size sanity
    if (overrideTileSize) {
        tileGrid = new WMTSTileGrid({
            tileSize: 512,
            extent: (_a = tileGrid.getExtent()) !== null && _a !== void 0 ? _a : undefined,
            origins: (_b = tileGrid.getOrigins()) !== null && _b !== void 0 ? _b : undefined,
    ...
    return {
        tileGrid,
        projection: /** @type {import("ol/proj/Projection.js").default} */ (info.projection),
    };
}
/**
...
function getProjectionFromAttributes(attributes) {
    ...
    if (projCode) {
        return /** @type {import("ol/proj/Projection.js").default} */ (getProjection(projCode));
    }
    const projDef = attributes['proj:projjson'] || attributes['proj:wkt2'];
    return /** @type {import("ol/proj/Projection.js").default} */ (fromProjectionDefinition(projDef));
}
/**
    ...
    return /** @type {import('zarrita').Readable} */ ({
        ...
        /** @type {import('zarrita').GetOptions|undefined} */ opts) {
            if (chunkCache.has(key)) {
                ...
                const cached = /** @type {Promise<Uint8Array|undefined>} */ (chunkCache.get(key));
                chunkCache.delete(key);
                chunkCache.set(key, cached);
                return cached;
            }
            const promise = /** @type {Promise<Uint8Array|undefined>} */ (store.get(/** @type {`/${string}`} */ (key), opts));
        ...
        getRange: /** @type {*} */ (_a = (store).getRange) === null || _a === void 0 ? void 0 : _a.bind(store),
    });
}
/**
...
function flipChunkRows(chunk) {
    ...
    for (let row = 0; row < rowCount; ++row) {
        data.set(source.subarray((rowCount - 1 - row) * colCount, (rowCount - row) * colCount), row * colCount);
    }
