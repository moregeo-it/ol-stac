...
function zarrCollection(id, description, bbox, asset, cube) {
	return {
		type: "Collection",
		stac_version: "1.1.0",
		stac_extensions: ["https://stac-extensions.github.io/datacube/v2.2.0/schema.json", "https://stac-extensions.github.io/projection/v2.0.0/schema.json"],
		id,
		description,
		license: "other",
		extent: {
...
var FTW_URL = "https://data.source.coop/ftw/global-data/predictions/zarr/collection.json";
var ZARR_V2 = "application/vnd.zarr; version=2";
...
var presets = {
	...
	"hurricane": { data: zarrCollection("hurricane-florence-era5", "ERA5 surface pressure during Hurricane Florence (Zarr v3, single resolution).", [
		-95.1,
		14.9,
		-39.9,
		45.1
	...
	"antarctic-era5": { data: zarrCollection("antarctic-era5-wind", "ERA5 wind speed over Antarctica (Zarr v3, custom polar stereographic projection).", [
		-180,
		-85,
		180,
		-60
		...
		dimensions: {
			y: {
				type: "spatial",
				axis: "y",
				extent: [-3316902, 3315371]
			},
			x: {
				type: "spatial",
				axis: "x",
				extent: [-3315363, 3316909]
			}
		},
	...
	"polar": {
		...
		data: zarrCollection("thwaites-ice-velocity", "Antarctic ice velocity subset for Thwaites Glacier (Zarr v2, EPSG:3031).", [
			-130.6,
			-78.6,
			-105.9,
			-71.9
			...
			dimensions: {
				y: {
					type: "spatial",
					axis: "y",
					extent: [-983450, -491950]
				},
				x: {
					type: "spatial",
					axis: "x",
					extent: [-1720950, -1147450]
				}
			},
			variables: { velocity: {
				type: "data",
	...
	"fgco2": { data: zarrCollection("delta-fg-co2", "OAE efficiency delta FG CO2 (Zarr v2, south-up, no spatial metadata in the store).", [
		-180,
		-85,
		180,
		85
	...
	"carbonplan-4d": {
		bands: ["tavg"],
		data: zarrCollection("carbonplan-4d", "Monthly average temperature and precipitation (Zarr v2 ndpyramid, EPSG:3857).", [
			-180,
			-85,
			180,
			85
	...
	"cmip6-tasmax": { data: zarrCollection("cmip6-tasmax", "CMIP6 daily maximum temperature, ACCESS-CM2 historical (Zarr v2 ndpyramid).", [
		-180,
		-85,
		180,
		85
	...
	"tos-con": { data: zarrCollection("ocean-tos-con", "Ocean surface temperature from the NOC eORCA1 model (Zarr v3 pyramid, EPSG:3857).", [
		-180,
		-85,
		180,
		85
...
function updateLayer() {
	...
	layer.on("sourceready", () => {
		map.getView().fit(layer.getExtent(), { padding: [
			20,
			20,
			20,
			20
...
//# sourceMappingURL=geozarr-datacube.js.map